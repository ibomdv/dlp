...
# CONSTANTS
# ══════════════════════════════════════════════════════
APP_TITLE       = "DRMLogPlotter rebuild - Version 1.21"
...
class DRMPlotter:
    ...
    def _build_bottom_bar(self, parent):
        ...
        tk.Button(uf_btn_row, text='Update', font=('Arial',8,'bold'),
                  bg='#aaddaa', width=6,
                  command=self._update_files).pack(side=tk.LEFT, padx=(0,1))
    ...
    def _update_stats(self, rows=None):
        ...
        # SNR + SYNC reading shows up, the "...0000" entry is treated as a
        # real, valid reception period starting at that recovery moment.
        ...
        while i < n:
            ...
            if mc.endswith('0000'):
                ...
                # the first row where SNR >= SNR_VALID_DB AND SYNC == 1.
                ...
                for k in range(run_start, run_end):
                    snr_k = _row_snr(valid_rows[k][1])
                    if snr_k is not None and snr_k >= SNR_VALID_DB and _row_sync_ok(valid_rows[k][1]):
                        recovery_idx = k
                        break
                if recovery_idx is not None:
                    # Same anti-glitch rule as normal modes: the validated
                    # remainder of this run must still last >= MIN_MODE_SECS.
                    if (run_end - recovery_idx) >= MIN_MODE_SECS:
                        confirmed.append((mc, valid_rows[recovery_idx][1]))
                # else: no SNR/SYNC recovery found anywhere in this run
                # — genuine no-signal gap, same as the previous behaviour.
            else:
    ...
    def _draw_right_labels(self, ax):
        ...
        # Readability fix: bright green (COL_DOPPLER) barely stands out
        # against a Gray frame background. Only in that one case, use the
        # existing darker green (COL_DOPPLER_TEXT) for the scale labels
        # and tick marks. All other frame backgrounds keep the original
        # bright green untouched.
        dop_scale_color = COL_DOPPLER_TEXT if self.cfg.get('frame_bg', 'darkblue') == 'gray' else COL_DOPPLER
            ...
            if p:
                with open(p,'w',encoding='utf-8') as f:
                    f.write(f'DRM Log Summary\nLabel: {log.label}\nSNR max: {fmt(sn_max)} dB  min: {fmt(sn_min)} dB  avg: {fmt(sn_avg)} dB\n'
                            ...
                            f'Main Service Channel: {self.v_msc.get()}\nProtection Level: {self.v_pl.get()}\n'
                            f'Audio Codec: {self.v_audio_codec.get()}\nProt. / Audio: {self.v_audio_mode.get()}\n'
    ...
    def _open_setup(self):
        ...
        # Set Event Info
        fsei=tk.LabelFrame(P,text='Set Event Info',bg=GUI_BG,font=('Arial',8,'bold'))
    ...
    def _show_dream_info_window(self):
        ...
        info_dlg.title('Information for DReaM use with "DRM LogPlotter rebuild"')
        info_dlg.configure(bg='#d4d0c8')
        info_dlg.resizable(True, True)
        center_dialog(info_dlg, self.root, 600, 640)
        ...
        # ── "Stop Dream (Button in this Window and in Main GUI)" ─────────
        # "Main GUI" highlighted in red — needs a Text widget with tags,
        # a plain Label can only show one colour for the whole string.
        stop_txt = tk.Text(I, bg='#d4d0c8', font=('Arial', 9),
                           height=2, wrap=tk.WORD, bd=0,
                           highlightthickness=0, padx=20)
        stop_txt.insert('end', '  Stop Dream  (Button in this Window and in ')
        stop_txt.insert('end', 'Main GUI', 'red_bold')
        stop_txt.insert('end', ')\n')
        stop_txt.insert('end',
            '  \u2192 Stops all processes correctly. dream.ini reset to normal.')
        stop_txt.tag_configure('red_bold', foreground='#cc0000',
                               font=('Arial', 9, 'bold'))
        stop_txt.configure(state=tk.DISABLED)
        stop_txt.pack(padx=0, pady=(0, 2), fill=tk.X)
        ...
        tk.Checkbutton(I,
                       text="Show this information at Set-Event start",
                       variable=show_again,
                       bg='#d4d0c8',
        ...
        tk.Button(I, text='OK — understood',
    ...
    def _set_event(self):
        ...
        dlg.configure(bg=GUI_BG)
        dlg.resizable(False, False)
        center_dialog(dlg, self.root, 780, 660)
        dlg.grab_set()
        dlg.transient(self.root)   # v_rig_test_06: keep dialog above its parent
        dlg.lift()
        dlg.focus_force()
        ...
        def _write_dream_ini(enable_log):
            ...
            if not ini_path:
                try:
                    status_lbl.config(
                        text='Dream.ini not found — checked program folder '
        ...
        def set_to_log_freq():
            ...
            # convention). rigctl_freq is what actually goes to the
            # receiver — corrected for the SDR USB/LSB offset if that
            # correction is enabled; a no-op otherwise (see
            # _rigctl_freq_for_station() further below in this function).
            rigctl_freq = _rigctl_freq_for_station(freq)
            if not self.cfg.get('trx_enable', 0):
                status_lbl.config(
            ...
            if missing_fields:
                ...
                status_lbl.config(
                    text=f'Please configure Receiver Settings in RX Config '
                         f'first! (missing: {", ".join(missing)}, '
                         f'conn_mode={conn_mode_chk})',
                    fg='#cc0000')
                return
            # ── v_rig_test_05: raw-socket path for Network + Hamlib NET
            ...
            if self._is_netrigctl_mode(conn_mode_chk, model_id):
                if not net_host_chk or not net_port_chk:
                    status_lbl.config(
                        text='Please configure Host/Port in RX Config first!',
                        fg='#cc0000')
                    return
                try:
                    freq_hz = int(float(rigctl_freq) * 1000)
                except ValueError:
                    status_lbl.config(text='Invalid Log Frequency value.',
                                      fg='#cc0000')
                    return
                ...
                if ok:
                    ...
                    if rigctl_freq != freq:
                        status_lbl.config(
                            text=f'TRX set to {rigctl_freq} kHz (SDR offset '
                                 f'applied for station {freq} kHz) — {info}',
                            fg='#007700')
                    else:
                        status_lbl.config(
                            text=f'TRX set to {freq} kHz — {info}', fg='#007700')
                else:
                    _safe_led('led1', led1_c, led1_o, 'red')
                    status_lbl.config(
                        text=f'TRX error [{net_host_chk}:{net_port_chk}]: {info}',
                        fg='#cc0000')
                return
            ...
            try:
                ...
                _safe_led('led1', led1_c, led1_o, 'red')
                status_lbl.config(
                    text=f'TRX timeout — check port and baud rate  |  cmd: {cmd_str}',
                    fg='#cc0000')
            except Exception as ex:
                status_lbl.config(text=f'TRX error: {ex}', fg='#cc0000')
        ...
        # ══════════════════════════════════════════════════════════════
        # DRM-RADIO-LIST — non-modal "DX hunting" browser window.
        #
        # Only ever writes into freq_var and calls the EXISTING
        # set_to_log_freq() defined above — it never starts Dream and
        # never touches TX-Site selection. Nothing about that logic is
        # changed or duplicated here.
        # ══════════════════════════════════════════════════════════════
        def _open_drm_radio_list():
            rl = tk.Toplevel(dlg)
            rl.title('DRM-Radio-List')
            rl.configure(bg=GUI_BG)
            rl.transient(dlg)      # associated with Set-Event dialog...
            # ...but deliberately NO grab_set(): this window stays
            # non-modal so the user can keep watching the main window
            # (SNR plot) while it's open.
            center_dialog(rl, self.root, 920, 700)
            ...
            last_choice_frame = tk.Frame(top, bg=GUI_BG)
            ...
            tk.Label(last_choice_frame, textvariable=last_choice_var, bg=GUI_BG,
                     font=('Arial',12,'bold'), fg='#008800').pack(
            ...
            tk.Label(top, textvariable=path_var, bg=GUI_BG, font=('Arial',8),
                     fg='#555555').pack(side=tk.LEFT, padx=(8,0))
            ...
            def refresh_view():
                ...
                for e, active_now in rows:
                    tv.insert('', 'end', values=(
                        ...
                        tags=('active' if active_now else 'inactive',))
            ...
            # Close button — the only other side=LEFT widget at this level;
            ...
            tk.Button(rl, text='Close', font=('Arial',9), width=10,
                      bg='#dddddd',
                      command=lambda: _on_rl_close()).pack(
                      side=tk.LEFT, padx=(40,8), pady=(0,4))
            ...
            def _auto_refresh():
                if not _rl_alive[0]:
                    return
                refresh_view()
                _rl_after_id[0] = rl.after(60000, _auto_refresh)
            ...
            if self.drm_schedule:
                list_status.config(
                    text=f'{len(self.drm_schedule)} entries loaded from: {path_var.get()}',
                    fg='#007700')
                ...
                if entries:
                    self.drm_schedule = entries
                    list_status.config(
                        text=f'Loaded {len(entries)} entries from: {path_var.get()}',
                        fg='#007700')

            refresh_view()
            _rl_after_id[0] = rl.after(60000, _auto_refresh)

        tk.Button(fq_row, text='DRM-Radio-List', font=('Arial',10), width=15,
                  bg='#ffe0aa',
                  command=_open_drm_radio_list).pack(side=tk.LEFT, padx=(84,0))
        ...
        # _rigctl_freq_for_station() below — the actual controls for them
        # live in their own separate frame further down (under
        # 'Timer-Events'), not here. Kept decoupled deliberately: moving
        # that frame around does not require touching this function.

        def _rigctl_freq_for_station(station_khz_str):
            """
            Every frequency the user enters anywhere in this dialog (this
            field, a Timer-Event slot, a DRM-Radio-List click, a preset)
            is always the TRUE station/broadcast frequency — that is what
            Dream itself receives, unchanged. This function computes the
            one place that's different: what actually goes to rigctl. If
            the SDR USB/LSB correction is enabled, the signed offset
            (positive for USB, negative for LSB — see the sideband radio
            buttons below) is subtracted, so the receiver's widened
            filter sits centred on the true signal. No-op (returns the
            value unchanged) when the correction is off.
            """
                ...
                def _ui():
                    ...
                    if freq:
                        try:
                            ...
                            baud = self.cfg.get('trx_baud', '9600')
                            conn = self.cfg.get('trx_conn_mode', 'usb')
                            # 'freq' is the TRUE station frequency entered
                            # in this Timer-Event slot; rigctl needs the
                            # SDR-offset-corrected value (same helper the
                            # DRM-Radio-List and manual Set button use).
                            rigctl_freq = _rigctl_freq_for_station(freq)
                            # ── v_rig_test_05: raw-socket path for Network
                            # + Hamlib NET rigctl — skips rigctl entirely.
                            if self._is_netrigctl_mode(conn, mid) \
                               and self.cfg.get('trx_enable', 0):
                                nh = self.cfg.get('trx_net_host', '127.0.0.1')
                                np_ = self.cfg.get('trx_net_port', '4532')
                                fhz = int(float(rigctl_freq) * 1000)
                                ok, _info = self._netrigctl_socket_set_freq(
                                    nh, np_, fhz)
                                self._netrigctl_led_state = 'green' if ok else 'red'
                                if ok:
                                    _safe_led('led1', led1_c, led1_o, 'green')
                                ...
                                if rigctl and mid and self.cfg.get('trx_enable', 0):
                                    ...
                                    cmd_r = [rigctl,'-m',str(mid),
                                             '-r',port,'-s',baud]
                                    cmd_r += self._rigctl_setconf_args()
                                    cmd_r += ['F',str(fhz)]
        ...
        def accept_schedule():
            ...
            for idx in range(NUM_SLOTS):
                ...
                # ── Intra-slot midnight handling ───────────────────────
                # Stop before/equal to start on the same naive day means
                # the event itself spans midnight — push stop to the next
                # day. e.g. Start 23:50 / Stop 00:10 → stop_dt → tomorrow.
                if stop_dt <= start_dt:
                    stop_dt += timedelta(days=1)

                # ── There is no such thing as a Timer Event "in the past" ─
                # A start time only ever means the next real occurrence of
                # that clock time. A small tolerance allows very short-notice
                # events to still fire immediately (see clamp below); beyond
                # that tolerance, keep advancing the WHOLE slot (start AND
                # stop together, so the duration is preserved) by a full day
                # until start_dt is within tolerance of "now" or later.
                # This is what correctly produces the "once daily midnight
                # jump" — e.g. a 00:01-00:50 slot listed after a 20:00 slot
                # on the same calendar day is simply understood as tomorrow
                # 00:01-00:50, with no need to compare it to the previous slot.
                ...
                # Start may still be up to START_PAST_TOLERANCE_MIN minutes
                # in the past (e.g. a very short-notice event, or the user
                # re-accepts while the slot should already be running).
                # Clamp to "now + 1s" so the timer fires almost immediately
