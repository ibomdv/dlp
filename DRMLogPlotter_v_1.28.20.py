...
# CONSTANTS
# ══════════════════════════════════════════════════════
APP_TITLE       = "DRMLogPlotter rebuild - Version 1.28.1"
...
class DRMPlotter:
    def __init__(self, root):
        ...
        # Aug 2026, user request: distinguishes 'Dream is running' (any
        # Dream process, OS-level, could be started completely outside
        # this programme) from 'Dream is running AND we started it'.
        # Used by the Radio-List's collision guard: retuning is safe to
        # allow when we started Dream ourselves WITHOUT a log (nothing
        # is being recorded), but must still be blocked whenever Dream
        # was started outside this programme — we simply have no way to
        # know if an externally-started Dream is logging or not, so that
        # case stays exactly as cautious as before. Set True at every
        # place this programme itself starts Dream, reset to False at
        # every 'Stop Dream' action and at programme start (see the
        # matching self._dream_log_flag assignments throughout).
        self._dream_started_by_app = False
    ...
    def _build_header_bar(self):
        ...
        # Aug 2026, user request ("Header-Zeile Text-Größe Toggle"):
        # click the header label to toggle between two preset text
        # sizes. Both presets were measured (headless render test) to
        # produce the SAME overall bar height as today's default —
        # pady is reduced exactly enough to offset the taller font, so
        # the row itself never grows or shrinks by a single pixel.
        #   Size 1 (default, unchanged): Arial 11 bold, pady 6
        #   Size 2 (new, larger):        Arial 12 bold, pady 6 (same
        #   pady as Size 1, per explicit user request)
        self._header_size_presets = [
            {'font': ('Arial', 11, 'bold'), 'pady': 6},   # Size 1 (default)
            {'font': ('Arial', 12, 'bold'), 'pady': 6},   # Size 2 (larger)
        ]
        self._header_size_state = [0]   # index into presets above

        def _toggle_header_size(event=None):
            self._header_size_state[0] = 1 - self._header_size_state[0]
            preset = self._header_size_presets[self._header_size_state[0]]
            lbl.config(font=preset['font'], pady=preset['pady'])

        lbl.config(cursor='hand2')
        lbl.bind('<Button-1>', _toggle_header_size)
        ToolTip(lbl, 'Click for larger text / Click for standard text')
    ...
    def _build_bottom_bar(self, parent):
        ...
        # Aug 2026, user request ("UTC/Local toggle"): 'UTC:' label is
        # now clickable — toggles its own text between 'UTC:' and
        # 'Local:', and (via the shared _tx_time_mode list below) tells
        # the clock digits next to it whether to show UTC or the PC's
        # local system time. Only this Main-GUI clock gets this —
        # the two Radio-List clocks elsewhere are untouched.
        self._tx_time_mode = ['utc']
        tx_utc_lbl = tk.Label(tx_clock_row, text='UTC:', bg=GUI_BG,
                 font=('Arial',11), fg='black',
                 bd=0, highlightthickness=0, pady=0)
        tx_utc_lbl.pack(side=tk.LEFT, anchor='s', padx=(0,3),
                 pady=(0, 1 if _is_win_clock else 0))
        def _toggle_tx_time_mode(event=None):
            if self._tx_time_mode[0] == 'utc':
                self._tx_time_mode[0] = 'local'
                tx_utc_lbl.config(text='Local:')
            else:
                self._tx_time_mode[0] = 'utc'
                tx_utc_lbl.config(text='UTC:')
        tx_utc_lbl.config(cursor='hand2')
        tx_utc_lbl.bind('<Button-1>', _toggle_tx_time_mode)
        ToolTip(tx_utc_lbl, 'Switch to Local / Switch to UTC')
        # Clock digits: same navy blue as 'Main Log' values available via
        # a click on the digits themselves (clickable=True), and their
        # UTC/Local source is driven by the 'UTC:'/'Local:' label above
        # (time_mode=self._tx_time_mode, shared mutable list).
        self._build_text_clock(tx_clock_row, self._main_clock_alive,
                                font=('Arial',11,'bold'), tight=True,
                                time_mode=self._tx_time_mode,
                                clickable=True).pack(
    ...
    def _replot_inner(self):
        ...
        if snr and times:
            ...
            if getattr(self, '_show_snr_avg_line', False) and snr:
                ...
                # Aug 2026, user request ("avg einen winzigen Tick
                # zurück"): offset reduced from 30 to 26 points — with
                # 2-digit averages (e.g. 'avg 22.5', one character
                # longer than 'avg 8.5') the last digit was sitting too
                # close to the canvas edge, per user screenshot. Purely
                # a text-offset change (xytext), does NOT touch the
                # axhline itself or the Main-Plot canvas/frame width —
                # annotation_clip stays False, same as before.
    ...
    def _radio_list_start_autoplot_after_log(self, log_dir, delay_ms=20000):
        ...
        def _fire():
            ...
            try:
                ...
                self.v_logs_count.set(str(len(self.all_logs)))
                # Aug 2026, user request ("Log-File Size wird nicht
                # angezeigt") — this auto-reload path (Radio-List ->
                # 'Start Dream with Log' -> AutoPlot) updated 'Logs:'
                # but forgot 'Size:'. Same calculation as the proven
                # manual-Update path in _load_files().
                _size_kb = sum(os.path.getsize(p) // 1024
                               for p in [derived_txt, derived_csv]
                               if os.path.exists(p))
                self.v_logs_size.set(f'{max(1, _size_kb)} kB')
            except Exception:
    ...
    def _open_drm_radio_list_window(self, freq_var=None, parent=None, on_freq_sent=None):
        ...
        def on_row_select(event=None):
            ...
            if not vals:
                return
            # Collision guard (user requirement, Aug 2026, refined):
            # retuning must still be blocked whenever a LOG could be
            # running — but if Dream is running WITHOUT a log AND this
            # programme itself started it (self._dream_started_by_app),
            # there is nothing to protect: no log is being written, and
            # we know for certain (we started it) that it isn't one
            # started externally with logging we can't see. A Dream
            # process we did NOT start ourselves still blocks exactly as
            # before — we have no way to know whether an externally-
            # started Dream is logging, so that case stays as cautious
            # as it always was.
            _dream_free_and_ours = (self._dream_started_by_app
                                     and not self._dream_log_flag)
            if self._is_dream_running() and not _dream_free_and_ours:
        ...
        def preset_click(i):
            if not presets[i]:
                list_status.config(
                    ...
                    fg='#cc6600')
                return
            # Same refined collision guard as on_row_select() above (Aug
            # 2026) — retuning via a preset is allowed when Dream is
            # running without a log AND was started by this programme
            # itself; still blocked for any logging session, and still
            # blocked for a Dream process this programme didn't start.
            _dream_free_and_ours = (self._dream_started_by_app
                                     and not self._dream_log_flag)
            if self._is_dream_running() and not _dream_free_and_ours:
                try:
        ...
        refresh_view()
        # Aug 2026, user request ("Radio-List öffnet auf letzter Wahl"):
        # scroll the freshly-built list so the row matching the
        # persisted last-chosen frequency (drm_radio_list_last_choice)
        # is immediately visible — that row is already colour-tagged
        # 'lastfreq' (light blue) by refresh_view() above, this just
        # brings it into view instead of leaving the user to scroll
        # down manually. Deliberately uses ONLY tv.see() — never
        # tv.selection_set() — because selecting a row fires the same
        # '<<TreeviewSelect>>' event as a manual click (on_row_select),
        # which would immediately re-send the frequency to the rig via
        # rigctl and could trigger the 'Stop Dream first' collision
        # guard the instant the window opens. tv.see() only scrolls;
        # it fires no event and sends nothing — confirmed by isolated
        # testing. First-ever use (no last choice saved yet) falls
        # through untouched: no match is found, so the list simply
        # opens at the top exactly as before.
        _last_khz_str = str(self.cfg.get(
            'drm_radio_list_last_choice', '') or '').strip()
        if _last_khz_str:
            for _iid in tv.get_children():
                _vals = tv.item(_iid, 'values')
                if _vals and str(_vals[2]) == _last_khz_str:
                    try:
                        tv.see(_iid)
                    except Exception:
                        pass
    ...
    def _build_text_clock(self, parent, alive_flag, font=('Arial', 14, 'bold'),
                           tight=False, time_mode=None, clickable=False):
        """
        ...
        time_mode: Aug 2026, user request ("UTC/Local toggle") — optional
        SHARED mutable ['utc'] / ['local'] list. When None (default, the
        two existing Radio-List call sites), the clock always shows UTC,
        completely unchanged. When supplied by the caller (Main-GUI TX
        clock only), the caller's own 'UTC:'/'Local:' label toggles this
        list's [0] value on click, and the running _tick() loop here
        picks that up on its very next 1s tick and switches between
        datetime.now(timezone.utc) and plain datetime.now() (the PC's
        local system time) accordingly — no restart of the tick loop
        needed.

        clickable: Aug 2026, user request ("colour toggle") — when True,
        binds a left-click on the clock digits themselves to toggle their
        colour between the existing green (#008800) and the same navy
        blue used for values in the 'Main Log' frame (#000080, easy to
        read there per user feedback), plus a ToolTip hint. Default
        False — the two existing Radio-List clocks are unaffected.
        ...
        if clickable:
            # Same navy blue as the 'Main Log' frame's value fields
            # (fg='#000080') — confirmed by user as easy to read.
            _clock_colour = ['green']
            def _toggle_clock_colour(event=None):
                if _clock_colour[0] == 'green':
                    _clock_colour[0] = 'blue'
                    lbl.config(fg='#000080')
                else:
                    _clock_colour[0] = 'green'
                    lbl.config(fg='#008800')
            lbl.config(cursor='hand2')
            lbl.bind('<Button-1>', _toggle_clock_colour)
            ToolTip(lbl, 'Klick for Blue / Klick for Green')

        def _tick():
            if not alive_flag[0]:
                return   # window closed — let this loop end itself
            if time_mode is not None and time_mode[0] == 'local':
    ...
    def _start_dream_with_log_from_main(self, freq_khz):
        ...
        try:
            ...
            self._dream_started_by_app = True
                            ...
                            try:
                                ...
                                if self.all_logs:
                                    ...
                                    self._set_led(self.led_l, self._led_l,
                                                  bool(self.all_csv))
                                    self.v_logs_count.set(str(len(self.all_logs)))
                                    # Aug 2026, user request ("Log-File
                                    # Size wird nicht angezeigt") — this
                                    # Scheduled-Event AutoPlot auto-reload
                                    # path updated 'Logs:' but forgot
                                    # 'Size:'. Same calculation as the
                                    # proven manual-Update path in
                                    # _load_files().
                                    _size_kb = sum(
                                        os.path.getsize(p) // 1024
                                        for p in [derived_txt, derived_csv]
                                        if os.path.exists(p))
                                    self.v_logs_size.set(
                                        f'{max(1, _size_kb)} kB')
    ...
    def _print_plot(self):
        ...
        def _detect_default_cups_printer():
            """
            Returns the CUPS default printer's queue name, or '' if
            none is set / lpstat isn't available.

            Aug 2026, user request ("sprachunabhängig") — previously
            this only stripped the English 'system default
            destination:' prefix, so on a non-English system (e.g.
            German CUPS: 'Keine systemvoreingestellten Ziele...') the
            raw human-readable sentence leaked unfiltered into the
            printer-name field instead of leaving it blank. Fix: take
            whatever follows the LAST colon — the 'label: value' shape
            holds regardless of language — then only accept that as a
            real printer name if it actually looks like one (a CUPS
            queue name never contains a space or sentence punctuation
            like '.'). Any 'no default destination' message, in any
            language, is rejected by that shape check alone — no need
            to hardcode every language's wording.
            """
            try:
        ...
        if sys.platform.startswith('win'):
            # Windows: native dialog handles printer selection — the
            # OS's own "Print" dialog (opened via os.startfile below)
            # already lists every installed printer for the user to
            # pick from, so no custom picker is built here. Adding one
            # ourselves would require a new external dependency
            # (pywin32) not used anywhere else in this programme, for
            # no real gain over what Windows already offers natively.
            tk.Label(step2,
            ...
            # Linux: real printer BROWSING (Aug 2026, user request) —
            # 'lpstat -p' lists every printer CUPS knows about; instead
            # of typing an exact name into a free-text field, the user
            # now gets a read-only dropdown of the actually installed
            # printers, with the system default pre-selected. Falls
            # back to the previous free-text field further below if no
            # printers can be detected (e.g. CUPS not running) — same
            # safety net already used for the default-printer lookup.
            try:
                default_printer = _detect_default_cups_printer()
            except Exception:
                default_printer = ''

            printer_list = []
            try:
                _lp_out = subprocess.check_output(
                    ['lpstat', '-p'],
                    stderr=subprocess.DEVNULL).decode()
                for _line in _lp_out.splitlines():
                    _line = _line.strip()
                    if _line.lower().startswith('printer '):
                        _parts = _line.split()
                        if len(_parts) >= 2:
                            printer_list.append(_parts[1])
            except Exception:
                printer_list = []

            if printer_list:
                tk.Label(step2, text='Select printer:', bg=GUI_BG,
                         font=('Arial', 9)).pack(padx=16, pady=(4, 2))
                _preselect = (default_printer if default_printer in printer_list
                              else printer_list[0])
                printer_var = tk.StringVar(value=_preselect)
                ttk.Combobox(step2, textvariable=printer_var,
                             values=printer_list, state='readonly',
                             font=('Arial', 10), width=26).pack(padx=16)
            else:
                # No printers detected — same free-text fallback as
                # before, nothing lost.
                tk.Label(step2,
            ...
            # macOS: unchanged — out of scope for this round (Windows +
            # Linux only, per user request). Always prints to the
            # system default printer further below; this field is
        ...
        try:
            ...
            err_txt = str(e).lower()
            no_printer_markers = (
                'no default', 'no destination', 'no application is associated',
                'not functioning', 'no printer', 'unable to locate', 'winerror 1155',
            )
            if any(marker in err_txt for marker in no_printer_markers):
                messagebox.showerror('Print Plot',
