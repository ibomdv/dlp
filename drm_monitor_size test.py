...
def draw_plot():
    ...
    ax_snr.set_ylabel("SNR (dB)", color=FG_WHITE, fontsize=9)
    ax_snr.tick_params(axis="y", colors=FG_WHITE, labelsize=9)
    ax_snr.tick_params(axis="x", colors=FG_WHITE, labelsize=9)
    ...
    ax_delay.tick_params(axis="y", colors=COLOR_DELAY, length=1)
    ax_delay.spines["right"].set_visible(False)
    ax_delay.spines["left"].set_position(("outward", 25))
    ax_delay.yaxis.set_label_position("left")
    ax_delay.yaxis.tick_left()
    ax_delay.set_ylabel("ms", color=COLOR_DELAY, fontsize=11)

    # Rechte Achse Frames (Blau) + Doppler Hz (Grün)
    ax_frames.set_ylabel("Frames", color=COLOR_AUDIO, fontsize=10)
    ax_frames.tick_params(axis="y", colors=COLOR_AUDIO, labelsize=9)
...
def make_frame(parent, title, col, padx=(2,2), pady=2):
    ...
    hdr = tk.Label(inner, text=title, bg=BG_FRAME, fg=FG_GRAY,
                   font=("Courier New",11,"bold"), anchor="w", padx=4)
    hdr.pack(fill="x")
    return inner
...
def row_kv(parent, key, val_text="–", val_style="Value.TLabel", r=0):
    ...
    tk.Label(f, text=key, bg=BG_PANEL, fg=FG_GRAY,
             font=("Courier New",11), width=16, anchor="w").pack(side="left")
