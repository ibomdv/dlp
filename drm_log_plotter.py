...
# CONSTANTS
# ══════════════════════════════════════════════════════
APP_TITLE       = "DRMLogPlotter rebuild - Version 1.00"
...
class DRMPlotter:
    ...
    def _build_top_info(self, parent):
        ...
        for i,(txt,var) in enumerate([('Label:',self.v_label),('Frequency:',self.v_freq),
            ...
            tk.Label(lc,text=txt,bg=GUI_BG,font=('Arial',10),anchor='w',width=11).grid(row=i,column=0,sticky='w',pady=1)
            tk.Label(lc,textvariable=var,bg=GUI_BG,font=('Arial',10,'bold'),
                     fg='#000080',anchor='w',width=16).grid(row=i,column=1,sticky='w',pady=1)
        ...
        for i,(txt,var) in enumerate([('Mode / Bandwidth:',self.v_mode),
            ...
            tk.Label(rc,text=txt,bg=GUI_BG,font=('Arial',10),anchor='w',width=16).grid(row=i,column=0,sticky='w',pady=1)
            tk.Label(rc,textvariable=var,bg=GUI_BG,font=('Arial',10,'bold'),
                     fg='#000080',anchor='w',width=12).grid(row=i,column=1,sticky='w',pady=1)
    ...
    def _schedule_dream_audio_read(self, log_dir, start_time_str, freq_khz_str,
        ...
        still running AND no real codec/protection has been found yet.
        ...
        Once real data is found (codec/protection not both '—'), the loop
        stops rescheduling itself — no need to keep polling after success.
    ...
    def _open_setup(self):
        ...
        dlg.configure(bg=GUI_BG)
        center_dialog(dlg, self.root, 780, 680)
        dlg.grab_set()
        dlg.transient(self.root)   # v_rig_test_06: keep dialog above its parent
        dlg.lift()
        dlg.focus_force()
    ...
    def _open_rx_config(self):
        ...
        center_dialog(rx_dlg, self.root, 760, 760)
        rx_dlg.minsize(600, 400)
        rx_dlg.grab_set()
    ...
    def _show_dream_info_window(self):
        ...
        center_dialog(info_dlg, self.root, 600, 700)
        info_dlg.grab_set()
        ...
        ttk.Separator(info_dlg, orient='horizontal').pack(
        ...
        tk.Label(info_dlg, text=info_text, bg='#d4d0c8',
        ...
        tk.Label(info_dlg, text='Dream start modes \u2014 overview:',
        ...
        tk.Label(info_dlg, text=modes_text, bg='#d4d0c8',
                 font=('Arial', 9), justify=tk.LEFT,
        ...
        stop_txt = tk.Text(info_dlg, bg='#d4d0c8', font=('Arial', 9),
        ...
        tk.Label(info_dlg, text=warn_text, bg='#d4d0c8',
                 font=('Arial', 9), justify=tk.LEFT,
                 anchor='w').pack(padx=20, pady=0, fill=tk.X)
        ...
        tk.Button(info_dlg, text='OK — understood',
        ...
        def _do_start(freq_khz, enable_log):
            ...
            try:
                ...
                if enable_log:
                    ...
                    # Frequency — from input field, then config, then ''
                    _fq_raw = freq_var.get().strip()
                    if not _fq_raw:
