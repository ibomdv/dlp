...
# CONSTANTS
# ══════════════════════════════════════════════════════
APP_TITLE       = "DRM-Log Plotter rebuild ( v.0.99c - beta experimental )"
...
class DRMPlotter:
    ...
    def _update_stats(self, rows=None):
        ...
        # Build list of (mode_code, row) — skip X0000 entries first
        valid_rows = []
        for r in rows:
        ...
        while i < len(valid_rows):
            mc, r = valid_rows[i]
            # Always skip X0000 — no signal
            if mc.endswith('0000'):
                i += 1
                continue
            # Count consecutive occurrences of this mode
            count = 0
            msc_ok = False
            j = i
            while j < len(valid_rows) and valid_rows[j][0] == mc:
                if count == 0:
                    # Check MSC on the first occurrence
                    try:
                        msc_ok = int(valid_rows[j][1].get('MSC', 0)) == 1
                    except Exception:
                        msc_ok = False
                count += 1
                j += 1
    ...
    def _replot_inner(self):
        ...
        if self.opt_delay.get() and delay:
            ax.plot(xs,[min(v,10.0) for v in delay],color=COL_DELAY,linewidth=0.6,zorder=3)
    ...
    def _open_setup(self):
        ...
        dlg.configure(bg=GUI_BG)
        center_dialog(dlg, self.root, 660, 680)
        dlg.grab_set()
    ...
    def _open_rx_config(self):
        ...
        center_dialog(rx_dlg, self.root, 760, 680)
        rx_dlg.grab_set()
