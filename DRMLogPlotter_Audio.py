...
def linux_atspi():
    ...
    # Confirmed by testing (July 2026): AT-SPI walks Dream's ENTIRE
    # accessibility tree regardless of whether a window/dialog is
    # currently visible on screen — this picks up long, irrelevant
    # strings (e.g. audio-device descriptions from a closed Settings
    # dialog, 90+ characters, containing "Stereo" as a substring) that
    # then falsely match is_mode_label()'s substring search (needed for
    # the OCR path's combined-line detection, e.g. "...xHE-AACMono").
    # A max length filter here removes that noise while comfortably
    # covering every real AT-SPI label fragment we've seen (longest is
    # "10.72 kbps EEP", 15 chars). Deliberately scoped to ONLY this
    # AT-SPI text-collection step — the OCR path already only ever reads
    # small, purpose-cropped label regions, so it never encounters this
    # kind of long unrelated text and does not need this filter.
    MAX_ATSPI_LABEL_LEN = 25
    try:
        import pyatspi
        ...
        for i in range(desktop.childCount):
            ...
            try:
                ...
                def _walk(obj):
                    try:
                        name = obj.name or ''
                        if name and len(name) <= MAX_ATSPI_LABEL_LEN:
                            labels.append(name)
                        try:
                            ti   = obj.queryText()
                            text = ti.getText(0, ti.characterCount)
                            if text and len(text) <= MAX_ATSPI_LABEL_LEN \
