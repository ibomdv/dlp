...
def is_mode_label(text):
    t = text.upper().strip()
    # FIX (Aug 2026): confirmed root cause of a false 'Stereo' detection
    # on every single broadcast on Linux Mint — a plain substring check
    # ('MONO' in t or 'STEREO' in t) matched ANY UI element anywhere in
    # Dream's whole window that merely CONTAINED the word "Stereo" as
    # part of some constantly-present, unrelated text (e.g. a menu item
    # or setting) — and since assemble_from_labels() lets a later match
    # overwrite an earlier correct one, that constant element
    # deterministically won every time, regardless of what was actually
    # playing.
    #
    # A plain revert to a strict exact match (t in ('MONO','STEREO'), as
    # used for weeks before this regression) closes that hole, but was
    # confirmed by testing to reopen a DIFFERENT, later-added case: the
    # OCR pipeline can legitimately return one combined status line like
    # "11.64 kbps EEP aac Mono", where "Mono" is no longer alone in the
    # string — an exact match would then miss it entirely.
    #
    # This version accepts BOTH legitimate cases while excluding the
    # false-positive one: the label is a real mode label only if EITHER
    # (a) it is exactly "MONO"/"STEREO" and nothing else (the narrow,
    # already-proven-safe original case), OR (b) it contains the word
    # AND is also clearly part of a genuine combined status line (i.e.
    # it also mentions KBPS or AAC, exactly like a real Dream status
    # line does). A stray, unrelated menu/setting label containing
    # "Stereo" has neither KBPS nor AAC alongside it, so it no longer
    # qualifies either way.
    if re.search(r'P[\-\s]*STEREO|PARAMETRIC', t):
        return True
    if t in ('MONO', 'STEREO'):
        return True
    if ('MONO' in t or 'STEREO' in t) and ('KBPS' in t or 'AAC' in t):
        return True
    return False
