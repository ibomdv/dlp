...
# CONSTANTS
# ══════════════════════════════════════════════════════
APP_TITLE       = "DRMLogPlotter rebuild - Version 1.22"
...
class DRMPlotter:
    ...
    def _build_bottom_bar(self, parent):
        ...
        tk.Button(tx_btn_row, text='Radio List', font=('Arial',9), width=_radio_list_width,
                  command=lambda: self._open_drm_radio_list_window()
                  ).pack(side=tk.LEFT)
    ...
    def _open_setup(self):
        ...
        # ── Validate: max 2 digits only — same pattern already used for
        # the Timer-Events time fields elsewhere in the programme. ──────
    ...
    def _open_drm_radio_list_window(self, freq_var=None, parent=None, on_freq_sent=None):
        ...
        #   [ Stop Dream  ]                   [ Close  ]
        # Close sits in the SAME grid row as 'Stop Dream' (row=1) — by
        # construction, its vertical position always matches that row
        # exactly, regardless of button/font sizing on any platform.
    ...
    def _set_event(self):
        ...
        def set_to_log_freq():
            ...
            status_lbl.config(text=msg, fg=fg)
